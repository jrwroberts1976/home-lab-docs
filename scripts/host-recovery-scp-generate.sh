...
OS_SUMMARY="$(grep -E '^(PRETTY_NAME|NAME|VERSION)=' "$INV_DIR/os-release.txt" 2>/dev/null | head -3 | tr '\n' ' ' || true)"
ARCH="$(grep -v '^#' "$INV_DIR/uname.txt" 2>/dev/null | tail -1 | awk '{print $NF}' || true)"
PKG_COUNT="$(count_lines packages-installed.txt)"
MANUAL_COUNT="$(count_lines packages-manual.txt)"
DOCKER_COUNT="$(grep -vc '^#' "$INV_DIR/docker-ps.txt" 2>/dev/null || echo 0)"
COMPOSE_COUNT="$(grep -cv '^#' "$INV_DIR/docker-compose-files.txt" 2>/dev/null || echo 0)"
REPO_COUNT="$(grep -cv '^#' "$INV_DIR/project-repos.txt" 2>/dev/null || echo 0)"
SECRET_COUNT="$(grep -cv '^#' "$INV_DIR/secret-file-inventory.txt" 2>/dev/null || echo 0)"

cat > "$OUT" <<EOF
# SCP — Recovery of $HOST
...
- OS evidence: **${OS_SUMMARY:-see os-release.txt}**
- Installed packages recorded: **$PKG_COUNT**
- Manually selected packages recorded: **$MANUAL_COUNT**
- Docker/container rows recorded: **$DOCKER_COUNT**
- Compose files found: **$COMPOSE_COUNT**
- Git repositories found: **$REPO_COUNT**
- Secret/key file locations inventoried: **$SECRET_COUNT**
