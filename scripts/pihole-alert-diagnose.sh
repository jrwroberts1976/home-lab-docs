...
query "($METRIC) > time() - $LOOKBACK" | python3 - "$NOW" <<'PY'
...
query "count by (host,category) (($METRIC) > time() - $LOOKBACK)" | python3 - <<'PY'
import json,sys
d=json.load(sys.stdin)
r=d.get('data',{}).get('result',[])
if not r:
    ...
    for x in sorted(r, key=lambda y:(y['metric'].get('host',''),y['metric'].get('category',''))):
        m=x['metric']; print(f"host={m.get('host','?')} category={m.get('category','?')} active_series={x['value'][1]}")
PY

printf '\n## Newest event age by host/category\n'
query "time() - max by (host,category) ($METRIC)" | python3 - <<'PY'
import json,sys
d=json.load(sys.stdin)
r=d.get('data',{}).get('result',[])
if not r:
    ...
    for x in sorted(r, key=lambda y:(y['metric'].get('host',''),y['metric'].get('category',''))):
        m=x['metric']; print(f"host={m.get('host','?')} category={m.get('category','?')} newest_age_seconds={float(x['value'][1]):.1f}")
PY
