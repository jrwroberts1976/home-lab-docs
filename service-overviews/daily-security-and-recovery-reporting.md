...
As of **3 September 2026**, the management report is generated by the daily **07:40** management-email chain. The standalone 08:30 management-report timer is disabled.

```text
homelab-greenbone-email.timer
    -> homelab-greenbone-email.service
       -> Requires/After: homelab-secops-management-report.service
...
## Management email scheduling correction — 3 September 2026

On 1 September 2026 the management email source changed to:

`/var/lib/homelab-secops-report/management/latest.md`

The email still ran at 07:40 while the management report was generated independently at 08:30. The sender correctly refused the previous day's stale report on 2 and 3 September.

The permanent correction is:

- keep `homelab-greenbone-email.timer` at 07:40;
- make `homelab-greenbone-email.service` require and run after `homelab-secops-management-report.service`;
- retain the Pi-hole evidence `ExecStartPre`;
- disable `homelab-secops-management-report.timer`;
- retain the fail-closed freshness check;
- map `GOOD`, `ATTENTION`, and `ACTION REQUIRED` to `GREEN`, `AMBER`, and `RED`.

The deployable runtime authority is stored under:

`configuration/ids-01/security-reporting/`

and:

`scripts/homelab-greenbone-email`
